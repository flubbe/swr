...
struct drawable_object
{
    ...
    /** rotation. */
    ml::vec3 rotation_axis{1.0, 0.0, 0.0};
    float rotation_angle{0};

    ml::vec3 steering_axis{0.0, 1.0, 0.0};
    float steering_value{0};

    /** Object type. */
    car_part part{car_part::none};
  ...
  const char* filename)
{
    ...
    for(auto& shape: shapes)
    {
        ...
        for(std::size_t f = 0; f < shape.mesh.indices.size() / 3; f++)
        {
            ...
            {
                diffuse = {
                  colors::linear_to_srgb(materials[current_material_id].diffuse[0]),
                  colors::linear_to_srgb(materials[current_material_id].diffuse[1]),
                  colors::linear_to_srgb(materials[current_material_id].diffuse[2])};
            }
            else if(cs == colorspace::srgb)
            {
                diffuse = {
            ...
            for(int k = 0; k < 3; k++)
            {
                ...
                normal_buffer.emplace_back(ml::vec4{n[k], 0.f});

                // Combine normal and diffuse to get color.
                color_buffer.emplace_back(ml::vec4{diffuse, 1.f});
        ...
        // DEBUG print material diffuse colors.
        for(const auto& m: materials)
        {
            std::println("material diffuse color: {:.3f} {:.3f} {:.3f}", m.diffuse[0], m.diffuse[1], m.diffuse[2]);
        }
...
class demo_viewer : public swr_app::renderwindow
{
    ...
    float wheel_angular_accel{3};

    /** steering value in the range (-1, 1). */
    float steering_value{0};

    /** steering acceleration in normalized_angle/s. */
    float steering_speed{5};
...
public:
    ...
    bool create() override
    {
        ...
        swr::SetClearColor(0.2, 0.2, 0.4, 1.0);
        ...
        int cmd_show_wireframe = swr_app::application::get_instance().get_argument("--wireframe", 0);
        ...
        if(!flat_shader_id || !wireframe_shader_id || !font_shader_id)
        {
        ...
        // get file to open
        const std::string base_model_path = "../src/demos/cars/assets/cars/cop/";

        simdjson::ondemand::parser json_parser;
        auto json = simdjson::padded_string::load(base_model_path + "asset.json");
        if(json.error() != simdjson::error_code::SUCCESS)
        {
            throw std::runtime_error(
              std::format("Error while loading assets: {}", simdjson::error_message(json.error())));
        }
        auto json_doc = json_parser.iterate(json);

        if(json_doc["type"] != "car")
        {
            throw std::runtime_error(
              std::format("Unexpected asset type. Expected 'car', got '{}'.", std::string_view{json_doc["type"]}));
        }

        colorspace cs;
        std::string color_space;

        simdjson::error_code error = json_doc["model"]["properties"]["colorspace"].get(color_space);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        ...
        {
            throw std::runtime_error(
              std::format("Invalid asset file: Unknown color space '{}'.", color_space));
        }

        std::string front_left_wheel_model_path;
        std::string front_right_wheel_model_path;
        std::string back_wheels_model_path;
        std::string body_model_path;

        error = json_doc["model"]["wheel"]["front_left"]["path"].get(front_left_wheel_model_path);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        error = json_doc["model"]["wheel"]["front_right"]["path"].get(front_right_wheel_model_path);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        error = json_doc["model"]["wheel"]["back"]["path"].get(back_wheels_model_path);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        error = json_doc["model"]["body"]["path"].get(body_model_path);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        ...
        ml::mat4x4 front_left_wheel_transform;
        ml::mat4x4 front_right_wheel_transform;
        ml::mat4x4 back_wheels_transform;
        ml::mat4x4 body_transform;

        auto json_array_to_mat4x4 = [](simdjson::ondemand::array& json_array) -> ml::mat4x4
        {
            ml::mat4x4 mat;
            ...
            for(auto inner_val: json_array)
            {
                ...
                for(double v: inner)
                {
                    if(col >= 4)
                    {
                        throw std::runtime_error("Too many columns in matrix");
                    }
                    mat[row][col] = static_cast<float>(v);
                    ++col;
                }

                if(col != 4)
                {
                    throw std::runtime_error("Row does not have 4 elements");
                }
            ...
            if(row != 4)
            {
                throw std::runtime_error("Matrix does not have 4 rows");
            }

            return mat;
        };

        error = json_doc["model"]["wheel"]["front_left"]["transform"].get(json_array);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        front_left_wheel_transform = json_array_to_mat4x4(json_array);

        error = json_doc["model"]["wheel"]["front_right"]["transform"].get(json_array);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        front_right_wheel_transform = json_array_to_mat4x4(json_array);

        error = json_doc["model"]["wheel"]["back"]["transform"].get(json_array);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        back_wheels_transform = json_array_to_mat4x4(json_array);

        error = json_doc["model"]["body"]["transform"].get(json_array);
        if(error)
        {
            throw std::runtime_error(
              std::format("Invalid asset file: {}.", simdjson::error_message(error)));
        }
        body_transform = json_array_to_mat4x4(json_array);

        std::array<std::tuple<std::string, car_part, ml::mat4x4>, 4>
          model_data = {
            std::make_tuple(base_model_path + front_left_wheel_model_path, car_part::front_wheel, front_left_wheel_transform),
            std::make_tuple(base_model_path + front_right_wheel_model_path, car_part::front_wheel, front_right_wheel_transform),
            std::make_tuple(base_model_path + back_wheels_model_path, car_part::back_wheel, back_wheels_transform),
        ...
        {
            ml::vec3 model_bmin, model_bmax;
            auto obj_count = objects.size();

            if(false == LoadObjAndConvert(model_bmin, model_bmax, std::get<2>(data), cs, &objects, materials, textures, std::get<0>(data).c_str()))
            {
                throw std::runtime_error("LoadObjAndConvert failed.");
            }

            for(std::size_t i = obj_count; i < objects.size(); ++i)
            {
                objects[i].part = std::get<1>(data);
            }

            bmin.x = std::min(bmin.x, model_bmin.x);
            bmin.y = std::min(bmin.y, model_bmin.y);
            bmin.z = std::min(bmin.z, model_bmin.z);

            bmax.x = std::max(bmax.x, model_bmax.x);
            bmax.y = std::max(bmax.y, model_bmax.y);
            bmax.z = std::max(bmax.z, model_bmax.z);
        }
    ...
    void update(float delta_time) override
    {
        ...
        int num_keys{0};
        auto keyboard_map = SDL_GetKeyboardState(&num_keys);    // FIXME don't call every frame.

        if(keyboard_map[SDL_SCANCODE_LEFT] && keyboard_map[SDL_SCANCODE_RIGHT])
        {
            /* ignore */
        }
        else if(keyboard_map[SDL_SCANCODE_LEFT])
        {
            steering_value = std::min(1.f, steering_value + steering_speed * delta_time);
        }
        else if(keyboard_map[SDL_SCANCODE_RIGHT])
        {
            steering_value = std::max(-1.f, steering_value - steering_speed * delta_time);
        }
        else
        {
            if(steering_value > 0)
            {
                steering_value = std::max(0.f, steering_value - steering_speed * delta_time);
            }
            else if(steering_value < 0)
            {
                steering_value = std::min(0.f, steering_value + steering_speed * delta_time);
            }
        }

        if(keyboard_map[SDL_SCANCODE_UP] && keyboard_map[SDL_SCANCODE_DOWN])
        {
            /* ignore */
        }
        else if(keyboard_map[SDL_SCANCODE_UP])
        {
            wheel_angular_speed += wheel_angular_accel * delta_time;
        }
        else if(keyboard_map[SDL_SCANCODE_DOWN])
        {
            wheel_angular_speed -= wheel_angular_accel * delta_time;
        }

        wheel_angular_speed = std::clamp(wheel_angular_speed, -3.f, 10.f);

        if(keyboard_map[SDL_SCANCODE_SPACE])
        {
        ...
        for(auto& o: objects)
        {
            if(o.part == car_part::back_wheel
               || o.part == car_part::front_wheel)
            {
                o.rotation_angle = wheel_angle;
            }

            if(o.part == car_part::front_wheel)
            {
                o.steering_value = steering_value;
            }
        }
    ...
    void draw_delta_time(float delta_time)
    {
        ...
        str = "Stop     [SPACE]";
        font_rend.draw_string(font::renderer::string_alignment::left, str, 0 /* ignored */, 2 * h);
    }
