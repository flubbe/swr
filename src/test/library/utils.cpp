...
void check_pattern(const std::vector<std::byte>& mem, std::uint32_t offset, T pattern)
{
    ...
    for(i = 0; i < chunks; i++)
    {
        ...
        {
            BOOST_CHECK_EQUAL(mem_ptr[k], p[k]);
        }
    }
    ...
    for(std::size_t k = 0; i * sizeof(T) + k + offset < mem.size(); ++k)
    {
        ...
        BOOST_CHECK_EQUAL(mem_ptr[i * sizeof(T) + k + offset], p[k]);
    }
}
...
BOOST_AUTO_TEST_CASE(internal_pack)
{
    ...
    BOOST_CHECK_EQUAL((out32 & 0xff000000), 0);
    BOOST_CHECK_EQUAL((out32 & 0x00ff0000), 0xaa0000);
    BOOST_CHECK_EQUAL((out32 & 0x0000ff00), 0x1100);
    BOOST_CHECK_EQUAL((out32 & 0x000000ff), 0xcc);

    std::uint8_t* out_ptr = reinterpret_cast<std::uint8_t*>(&out32);
    BOOST_CHECK_EQUAL(out_ptr[3], 0);
    BOOST_CHECK_EQUAL(out_ptr[2], 0xaa);
    BOOST_CHECK_EQUAL(out_ptr[1], 0x11);
    BOOST_CHECK_EQUAL(out_ptr[0], 0xcc);

    std::uint64_t out64 = pack64(out32, 0xffeeddbb);
    BOOST_CHECK_EQUAL((out64 & 0xffffffff), 0xffeeddbb);
    BOOST_CHECK_EQUAL((out64 & 0xffffffff00000000), 0xaa11cc00000000);
    ...
    BOOST_CHECK_EQUAL(ptr128[3], 0xaa11cc);
    BOOST_CHECK_EQUAL(ptr128[2], 0xffeeddbb);
    BOOST_CHECK_EQUAL(ptr128[1], 0xaa11cc);
    BOOST_CHECK_EQUAL(ptr128[0], 0xaabbccaa);
#endif /* defined(SWR_USE_SIMD) */
}
